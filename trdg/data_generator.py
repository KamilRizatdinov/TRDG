...
class FakeTextDataGenerator(object):
    ...
    def generate(
        ...
        try:
            ...
            #######################
            # Image to the center of 512x512 #
            #######################

            new_image = None

            if background_type == 0:
                new_image = np.ones((512, 512)) * 255
                cv2.randn(new_image, 235, 10)
            ...
            paste_position = (
                (512 - final_image.width) // 2,
                (512 - final_image.height) // 2,
            )

            # Paste the original image onto the new white image
            new_image.paste(final_image, paste_position)
            ...
            if name_format == 0:
                ...
                name = "{}_{}".format(font.split("/")[-1].split(".")[0], str(index))
            else:
                print(
                    "{} is not a valid name format. Using default.".format(name_format)
                )
                name = "{}_{}".format(text, str(index))
            ...
            if out_dir is not None:
                new_image.save(os.path.join(out_dir, image_name))
                if output_mask == 1:
